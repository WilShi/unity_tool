...
class Player(QWidget):
    def __init__(self):
        ...
        self.textLable = QLabel('美好的一天从一首歌开始!')
        self.infoLabel = QLabel('测试版本V 1.0.0')
        ...
        self.downloadBtn.setStyleSheet("QPushButton{border-image: url(resource/image/download.png)}")
        self.downloadBtn.setFixedSize(24, 24)

        self.timer = QTimer(self)
        self.timer.start(1000)
        self.timer.timeout.connect(self.playByMode)

        self.hBoxSlider = QHBoxLayout() # 放置歌曲进度条
        ...
        self.hBoxButton = QHBoxLayout() # 放置按钮的位置
        ...
        self.vboxMain = QVBoxLayout() # 完整页面的布局
    ...
    def search(self):
        ...
        resultinfo = Search().get_music_id(keywords)
        self.musicList.clear()
        self.songs_list = []

        for i in resultinfo:
            self.songs_list.append([resultinfo[i], i])
            self.musicList.addItem(resultinfo[i])
    ...
    def download(self):
        ...
        if out_path:
            ...
            if type(song_id) == int:
                Songs().download(song_id, song_name, out_path)
                self.Tips(f"下载成功，路径为：{out_path}")
            else:
                self.Tips(f"该歌曲为本地文件，路径为{song_id}")
    ...
    def setCurPlaying(self):
        self.cur_playing_song = self.songs_list[self.musicList.currentRow()][-1]
        if type(self.cur_playing_song) == int:
            self.player.setMedia(QMediaContent(QUrl(f'http://music.163.com/song/media/outer/url?id={self.cur_playing_song}')))
        else:
            self.player.setMedia(QMediaContent(QUrl(self.cur_playing_song)))
        self.textLable.setText("当前播放："+self.songs_list[self.musicList.currentRow()][0])
